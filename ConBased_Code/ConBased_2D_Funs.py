...
def Connectivity(core_dict,i_num,j_num):
    ...
    box_data = np.zeros([x_max-x_min+1,y_max-y_min+1])
...
def Del_CP_Dict(core_data,peak_dict,core_dict,index,core_pixels,peak_delta,nearest_dist,region_pixels_min):
    item_core = -1
    item_peak = -1
    i_num = index
    ...
    if len(pdk_sort)>0:
        ...
        if connectivity > 1:
            ...
            logic = len(core_dict[i_num]) < core_pixels and peak_delta_0 > peak_delta\
                     or nearest_dis_0 < nearest_dist or len(core_dict[i_num]) < region_pixels_min
        if logic:
            core_dict[order] = core_dict[order] + core_dict[i_num]
            del core_dict[i_num]
            del peak_dict[i_num]
        else :
            if len(core_dict[i_num]) > region_pixels_min:
...
def Update_CP_Dict(peak_dict,core_dict,core_data,core_pixels,peak_delta,nearest_dist,region_pixels_min):
    key = 0
    core_dict_record = {}
    peak_dict_record = {}
    while len(core_dict.keys()) != 1 :
        length = len(core_dict.keys())
        if len(core_dict.keys()) > 1:
            ...
            for index_i in indexs[0]:
                ...
                core_dict,peak_dict,item_core,item_peak = \
                    Del_CP_Dict(core_data,peak_dict,core_dict,index,core_pixels,peak_delta,nearest_dist,region_pixels_min)
                if item_core != -1:
...
#         od_mass = od_mass - od_mass.min()
    ...
    detect_infor_dict['clump_maximum'] = np.array(clump_maximum).tolist()
